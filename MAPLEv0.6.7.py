...
#why does likelihood go down sometimes after an attempted second round of EM? Being investigated
...
# if reRootAtInternalNode==True , flip also mutations in the MAT. In this case sample represents the internal node whose parent branch to use as new root.
...
def stringForNode(tree,nextNode,nameNode,dist,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn):
	children=tree.children
	up=tree.up
	dist=tree.dist
	name=tree.name
	...
	if aBayesPlusActive or estimateMATon or printIQtreeSupportForNode or printIQtreeSupportForNode:
		if (up[nextNode]!=None and (dist or supportFor0Branches or errorsOn)) :
			stringList.append("[&")
			...
			if aBayesPlusActive and (dist or supportFor0Branches):
				...
				if networkOutput:
					...
					for iNode in range(len(alternativePlacements[nextNode])):
						stringList.append(name[alternativePlacements[nextNode][iNode][0]]+":"+str(alternativePlacements[nextNode][iNode][1]))
						if iNode<(len(alternativePlacements[nextNode])-1):
							stringList.append(",")
					stringList.append("}")
				if estimateMATon and (dist or errorsOn):
					stringList.append(",")
			if estimateMATon and (dist or errorsOn):
			...
			if direction==0:
				...
				if dist[nextNode] or binary or up[nextNode]==None:
					...
					if aBayesPlusOn or estimateMAT or performLineageAssignment:
						stringList.append(stringForNode(tree,nextNode,"",dist[nextNode],estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
					if dist[nextNode]:
			...
			if len(minorSequences[nextNode])>0 and includeMinorSeqs:
				if binary:
					...
					if supportForIdenticalSequences or performLineageAssignment:
						if namesInTree==None:
							stringList.append(stringForNode(tree,nextNode,name[nextNode],0.0,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
						else:
							stringList.append(stringForNode(tree,nextNode,namesInTree[name[nextNode]],0.0,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
					else:
						if namesInTree==None:
							stringList.append(name[nextNode])
						else:
					...
					for s2 in minorSequences[nextNode][:-1]:
						...
						if supportForIdenticalSequences or performLineageAssignment:
							if namesInTree==None:
								stringList.append(stringForNode(tree,nextNode,s2,0.0,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
							else:
								stringList.append(stringForNode(tree,nextNode,namesInTree[s2],0.0,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
						else:
							if namesInTree==None:
					...
					if supportForIdenticalSequences or performLineageAssignment:
						if namesInTree==None:
							stringList.append(stringForNode(tree,nextNode,minorSequences[nextNode][-1],0.0,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
						else:
							stringList.append(stringForNode(tree,nextNode,namesInTree[minorSequences[nextNode][-1]],0.0,estimateMAT=estimateMAT,networkOutput=networkOutput,aBayesPlusOn=aBayesPlusOn))
					else:
						if namesInTree==None:
...
def mergeVectors(probVect1,bLen1,fromTip1,probVect2,bLen2,fromTip2,returnLK=False,isUpDown=False,numMinor1=0,numMinor2=0):
	...
	if returnLK:
		...
		if usingErrorRate:
			#TODO fix this and below to account for minor sequences
...
def rootVector(probVect,bLen,isFromTip,tree,node):
	...
	while nodeList:
		node=nodeList.pop()
		if mutations[node]:
			newProbVect=passGenomeListThroughBranch(newProbVect, mutations[node], dirIsUp=False) #, modifyCurrentList=True
	shorten(newProbVect)
...
def updateBLen(tree,node,addToList=False,nodeList=None):
	cNode=node
	up=tree.up
		...
		if direction==0:
			if children[node]:
				...
				if firstSetUp:
					...
					#TODO removed minor sequences from the tree
...
def findBestParentTopology(tree,node,child,bestLKdiff,removedBLen,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,sequentialSearch=True):
	...
	if aBayesPlusOn and sequentialSearch:
		...
		totalTimeFindingParent[0]+=(time()-timeStartParentTopology)
		return bestNode, bestScore, bestBranchLengths, [], 1.0, bestRemovedPartials


#function traversing the tree to find the best new root.
def findBestRoot(tree,root,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology):
	up=tree.up
	children=tree.children
	mutations=tree.mutations
	minorSequences=tree.minorSequences
	...
	if children[bestNode]: # case there is only one sample outside of the subtree doesn't need to be considered
		...
		try:
			newLower, Lkcontribution=mergeVectors(vectUp1,dist[child2],isTip2,vectUp2,dist[child1],isTip1,returnLK=True,numMinor1=len(minorSequences[child2]),numMinor2=len(minorSequences[child1]))
				...
				print(dist[child1])
				print(isTip1)
				print(len(minorSequences[child2]))
				print(len(minorSequences[child1]))
				exit()
	...
	while nodesToVisit:
		...
		for i in range(2):
			...
			try:
				...
				print(mutations[t1])
				print(mutations[up[t1]])
				print(mutations[children[up[t1]][0]])
				print(mutations[children[up[t1]][1]])
				print(dist[t1])
				print(dist[up[t1]])
				print(dist[children[up[t1]][0]])
				print(dist[children[up[t1]][1]])
				print(probVect[t1])
				print(probVect[up[t1]])
				print(probVect[children[up[t1]][0]])
				print(probVect[children[up[t1]][1]])
				print(nodesVisitedRoot)
				exit()
...
if not doNotReroot:
	...
	newT1=findBestRoot(tree,t1,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology)
	if newT1!=t1:
...
for nRound in range(nRounds):
	...
	print("Writing preliminary tree to file: "+outputFile+"_round"+str(nRound+1)+"_preliminary_tree.tree")
	...
	while subRound<20:
		...
		print("Writing preliminary tree to file: "+outputFile+"_round"+str(nRound+1)+"_subround"+str(subRound+1)+"_preliminary_tree.tree")
		newickString=createNewick(tree,t1,binary=binaryTree,namesInTree=namesInTree,estimateMAT=False,networkOutput=False,aBayesPlusOn=False)
	...
	#newickString=createNewick(tree,t1,binary=binaryTree,namesInTree=namesInTree,includeMinorSeqs=False)
	#fileName=outputFile+fileNameAdd+"_tree_noMinorSeqs.tree"
	#file=open(fileName,"w")
	#file.write(newickString)
	#file.close()
