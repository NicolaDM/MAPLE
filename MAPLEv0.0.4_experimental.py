...
#TODO create function to estmate the total likelihood of a tree; then test on many small trees if the likelihood decreases after an SPR move and why.
#TODO similarly, after every SPR or branch length move recalculate all genome lists to test if any is incorrect in the current tree!
...
parser.add_argument("--thresholdLogLK",help="logLK difference threshold to consider a logLk close to optimal.",  type=float, default=40.0)
...
parser.add_argument("--debugging", help="Test that things are going as expected - only for debugging purposes.", action="store_true")
...
parser.add_argument("--thresholdDiffForUpdate",help="Consider the probability of a new partial changed if the difference between old and new if above this threshold.",  type=float, default=0.00001)
...
def areVectorsDifferentDebugging(probVect1,probVect2,threshold=0.1):
	if probVect2==None:
		print('none case')
		return True
	indexEntry1, indexEntry2, pos = 0, 0, 0
			...
			if entry1[0]<5:
				if len(entry1)>2:
					if abs(entry1[2] - entry2[2])>threshold:
						...
						return True
					if len(entry1)==4:
						if abs(entry1[3] - entry2[3])>threshold:
...
#TODO Fix so that polytomies are printed correctly, instead of printing branches with length "False"
		...
		if direction==0:
			...
			if lastNode==node.children[0]:
				...
				if newLower==None:
					...
					print(node.children[1].probVect)
					exit()
				elif areVectorsDifferentDebugging(node.probVect,newLower):
					print("Strange, while calculating tree likelihood encountered non-updated lower likelihood genome list: new list, old list, and children lists")
					print(newLower)
					print(node.probVect)
					print(node.children[0].probVect)
					print(node.children[1].probVect)
