...
#TODO test all the functions affected by error rates, trying them both with extremely low error rate and without errors and see which one ever gives different results and why.
...
parser.add_argument("--errorRateSiteSpecificFile", help="provide a file directory to a file that contains the siteSpecific error rates", type=str, default=None) #errorRateSiteSpecific
			...
			if m[0]=="n" or m[0]=="-":
				...
				if onlyNambiguities:
					...
					if usingErrorRate and (numMinorSeqs==0):
						...
						if errorRateSiteSpecific: errorRate = errorRates[currPos-1] #TODO changed from "entry[1]" to "entry[1]-1", check that this is right
...
def mergeVectorsUpDown(probVect1,bLenUp,probVect2,bLenDown):
	# global usingErrorRate #TODO remove
	if not errorRateSiteSpecific: 
		errorRate=errorRateGlobal
	...
	# if usingErrorRate:
	# 	usingErrorRate=False
	# 	probVect1non=makeVectorNonError(probVect1)
	# 	probVect2non=makeVectorNonError(probVect2)
	# 	otherProbVect=mergeVectorsUpDown(probVect1non,bLenUp,probVect2non,bLenDown)
	# 	usingErrorRate=True
	# 	if areVectorsDifferentespiteError(otherProbVect,probVect):
	# 		print("Difference in results with and without error in mergeVectorsUpDown")
	# 		print(probVect1)
	# 		print(bLenUp)
	# 		print(probVect2)
	# 		print(bLenDown)
...
def mergeVectors(probVect1,bLen1,probVect2,bLen2,returnLK=False):
				...
				if entry2[0]==6: #entry1 is a nucleotide and entry2 is "O"
					...
					if totLen2 or flag2:
						...
						for i in range4:
							newVec[i]=newVec[i]/sumV
							if newVec[i]<0:
								print("mergeVectors() different nucleotides, negative prob?")
								print(entry1)
								print(bLen1)
								print(entry2)
								print(bLen2)
	...
	# 	probVect2non=makeVectorNonError(probVect2)
	# 	if returnLK:
	# 		otherProbVect, otherLK=mergeVectors(probVect1non,bLen1,probVect2non,bLen2,returnLK=True)
	# 	else:
	# 		otherProbVect=mergeVectors(probVect1non,bLen1,probVect2non,bLen2)
	# 	usingErrorRate=True
	# 	if areVectorsDifferentespiteError(otherProbVect,probVect) or (returnLK and (abs(otherLK-cumulPartLk)>thresholdDiffForUpdate)):
	# 		print("Difference in results with and without error in mergeVectors")
	# 		print(probVect1)
	# 		print(bLen1)
	# 		print(probVect2)
	# 		print(bLen2)
...
def findProbRoot(probVect):
	...
	# if usingErrorRate:
	# 	usingErrorRate=False
	# 	probVectnon=makeVectorNonError(probVect)
	# 	otherlogLK=findProbRoot(probVectnon)
	# 	usingErrorRate=True
	# 	if (abs(otherlogLK-logLK)>thresholdDiffForUpdate):
	# 		print("Difference in LK with and without error in findProbRoot")
	# 		print(probVect)
	# 		print(logLK)
	# 		print(otherlogLK)
	# 		raise Exception("exit")
...
def rootVector(probVect,bLen):
	...
	# 	usingErrorRate=False
	# 	probVectnon=makeVectorNonError(probVect)
	# 	otherProbVect=rootVector(probVectnon,bLen)
	# 	usingErrorRate=True
	# 	if areVectorsDifferentespiteError(otherProbVect,newProbVect):
	# 		print("Difference in vector with and without error in rootVector")
	# 		print(probVect)
	# 		print(bLen)
	# 		print(otherProbVect)
	# 		print(newProbVect)
...
def estimateBranchLengthWithDerivative(probVectP,probVectC):
	...
	nZeros=0
	# foundDifference=False #TODO remove
	while True:
						...
						if len(entry1)==4+usingErrorRate:
							...
							if flag2:
								if mutMatrix[refIndeces[pos]][entry2[0]]:
									...
									# print(mutMatrices[pos])
									# print(mutMatrices[pos+1])
									# print(mutMatrices[pos-1])
									# print(entry1)
									# print(entry2)
									# print(pos)
	...
	if n==0:
		# if foundDifference:
		# 	print("returning 0 length in estimateBranchLengthWithDerivative when vectors are different 1")
		# 	print(probVectP)
		# 	print(probVectC)
		# 	print(c1)
		# 	print(ais)
		...
		if tDown<=0.0:
			# if foundDifference:
			# 	print("returning 0 length in estimateBranchLengthWithDerivative when vectors are different 2")
			# 	print(probVectP)
			# 	print(probVectC)
			# 	print(c1)
			# 	print(ais)
	...
	if vDown>c1+minBLenSensitivity or vUp<c1-minBLenSensitivity:
		if vUp<c1-minBLenSensitivity and (not tUp):
			# if foundDifference:
			# 	print("returning 0 length in estimateBranchLengthWithDerivative when vectors are different 3")
			# 	print(probVectP)
			# 	print(probVectC)
			# 	print(c1)
			# 	print(ais)
	...
	# if foundDifference and (not tUp):
	# 	print("returning 0 length in estimateBranchLengthWithDerivative when vectors are different 3")
	# 	print(probVectP)
	# 	print(probVectC)
	# 	print(c1)
	...
	# if usingErrorRate:
	# 	usingErrorRate=False
	# 	probVect1non=makeVectorNonError(probVectP)
	# 	probVect2non=makeVectorNonError(probVectC)
	# 	othertUp=estimateBranchLengthWithDerivative(probVect1non,probVect2non)
	# 	usingErrorRate=True
	# 	if ( (abs(othertUp-tUp)>thresholdDiffForUpdate)):
	# 		print("Difference in results with and without error in estimateBranchLengthWithDerivative")
	# 		print(probVectP)
	# 		print(probVectC)
	# 		print(othertUp)
	# 		print(tUp)
...
#TODO make sure terminal node partial likelihoods are recalculated after estimating the error rates.
def reCalculateAllGenomeLists(root, checkExistingAreCorrect=False,countNodes=False,countPseudoCounts=False,pseudoMutCounts=None,data=None,firstSetUp=False):
...
if inputTree!="":
	data2=reCalculateAllGenomeLists(tree1,countPseudoCounts=True,pseudoMutCounts=pseudoMutCounts,data=data,firstSetUp=True,moveDataToOtherDict=True)
...
def appendProbNode(probVectP,probVectC,bLen):
				...
				if entry2[0]==4:
					...
					if len(entry1)==4+usingErrorRate:
						...
						if flag1:
							...
							for i in range4:
								if i1==i:
									tot2=rootFreqs[i]*(1.0+mutMatrix[i][i]*entry1[2]-errorRate)
								else:
									tot2=rootFreqs[i]*(mutMatrix[i][i1]*entry1[2]+errorRate*0.33333)
								if contribLength:
									tot3=0.0
									for j in range4:
					...
					if len(entry1)==4+usingErrorRate:
						...
						if flag1 or flag2:
							if errorRateSiteSpecific: errorRate = errorRates[pos]
							if contribLength:
								totalFactor*=((rootFreqs[i1]*(mutMatrix[i1][i2]*contribLength + flag2*errorRate*0.33333)*(1.0+mutMatrix[i1][i1]*entry1[2]-flag1*errorRate)+rootFreqs[i2]*(mutMatrix[i2][i1]*entry1[2] + flag1*errorRate*0.33333)*(1.0+mutMatrix[i2][i2]*contribLength-flag2*errorRate))/rootFreqs[i1])
						else:
							if contribLength:
						...
						if flag2:
							if errorRateSiteSpecific: errorRate = errorRates[pos]
							totalFactor*=mutMatrix[refIndeces[pos]][entry2[0]]*contribLength+errorRate*0.33333
						else:
							if contribLength:
			...
			# entry1 is of type "O"
				...
				if entry2[0]==6:
					...
					if (usingErrorRate and (len(entry2)>2) and entry2[-1] ):
						if errorRateSiteSpecific: errorRate = errorRates[pos]
						totalFactor*=(entry1[-1][i2]*(1.0-1.33333*errorRate)+0.33333*errorRate+contribLength*(entry1[-1][0]*mutMatrix[0][i2]+entry1[-1][1]*mutMatrix[1][i2]+entry1[-1][2]*mutMatrix[2][i2]+entry1[-1][3]*mutMatrix[3][i2]))
					else:
						if contribLength:
				...
				if entry2[0]==entry1[0]:
					...
					if entry2[0]<5: #entry2 is a nucleotide
						...
						if len(entry1)==4+usingErrorRate:
							if flag1 or flag2:
								if errorRateSiteSpecific: errorRate = errorRates[pos]
								totalFactor*=(( rootFreqs[i1]*(mutMatrix[i1][i2]*contribLength + flag2*errorRate*0.33333)*(1.0+mutMatrix[i1][i1]*entry1[2]-flag1*errorRate) + rootFreqs[i2]*(mutMatrix[i2][i1]*entry1[2]+flag1*0.33333*errorRate)*(1.0+mutMatrix[i2][i2]*contribLength-flag2*errorRate) )/rootFreqs[i1])
							else:
								if contribLength:
									totalFactor*=(( rootFreqs[i1]*mutMatrix[i1][i2]*contribLength*(1.0+mutMatrix[i1][i1]*entry1[2]) + rootFreqs[i2]*mutMatrix[i2][i1]*entry1[2]*(1.0+mutMatrix[i2][i2]*contribLength) )/rootFreqs[i1])
								else:
							...
							if flag1 or flag2:
								if errorRateSiteSpecific: errorRate = errorRates[pos]
								totalFactor*=(mutMatrix[i1][i2]*contribLength+(flag1 + flag2)*0.33333*errorRate)
							else:
								if contribLength:
	...
	# 	probVect1non=makeVectorNonError(probVectP)
	# 	probVect2non=makeVectorNonError(probVectC)
	# 	otherLK=appendProbNode(probVect1non,probVect2non,bLen)
	# 	usingErrorRate=True
	# 	if (otherLK<-1000000) and ((Lkcost+log(totalFactor))<-100):
	# 		pass
	# 	elif ( (abs(otherLK-(Lkcost+log(totalFactor)))>thresholdDiffForUpdate)):
	# 		print("Difference in results with and without error in appendProbNode")
	# 		print(probVectP)
	# 		print(probVectC)
	# 		print(bLen)
	# 		print(otherLK)
	# 		print(Lkcost+log(totalFactor))
	# 		raise Exception("exit")
	return Lkcost+log(totalFactor)
	...
	while node!=None:
		if direction==0:
			...
			if lastNode==node.children[0]:
				...
				# print(node.children[1].probVect)
				# print(node.children[1].dist)
				# print(mergeVectors(node.children[0].probVect,node.children[0].dist,node.children[1].probVect,node.children[1].dist,returnLK=True))
...
def expectationMaximizationCalculationRates(root):
						...
						if entry1[0]==4: # case entry1 is R
							if entry2[0]==4:
								...
								pos+=1
																...
																#TODO check waitingTimesSites from here upwards
						# entry1 is of type "O"
						elif entry1[0]==6:
							if not totLen2:
								normalization=0.0
	...
	if useRateVariation:
		...
		for i in range(lRef):
			...
			if totExpected<0.0 or countsSites[i]<0.0: #TODO remove
				print("negative totExpected "+str(totExpected)+" or countsSites "+str(countsSites[i]))
...
# activate variation in substitution and error rates if estimating them
...
if inputTree=="" or largeUpdate or rateVariation or usingErrorRate:
	...
	#if using error rates, run EM iteratively until convergence in likelihood
...
if rateVariation:
	...
	for i in range(lRef):
		...
		#if siteRates[i]>20:
		#	print("Site "+str(i+1)+" with substitution rate "+str(siteRates[i]))
if estimateSiteSpecificErrorRate:
	file.write("\n\n"+"Site error rates:\n")
	for i in range(lRef):
		file.write(str(i+1)+"\t"+str(errorRates[i])+"\n")
		#if errorRates[i]>0.005:
		#	print("Site "+str(i+1)+" with error rate "+str(errorRates[i]))
elif estimateErrorRate:
