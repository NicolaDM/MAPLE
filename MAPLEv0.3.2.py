...
parser.add_argument("--aBayesPlus", help="Calculate branch support with an extension of the aBayes approach of Anisimova et al 2011.", action="store_true")
...
parser.add_argument("--minMutProb", help="Minimum mutation probability to be written to output when using option --estimateMAT", type=float, default=0.1)
	...
	while nextNode!=None:
		if nextNode.children:
			if direction==0:
				...
				stringList.append(")")
				if aBayesPlusOn or estimateMAT:
					if networkOutput:
						name=nextNode.name
					else:
						name=""
