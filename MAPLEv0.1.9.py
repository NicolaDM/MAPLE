...
parser.add_argument("--minBLenSensitivity",help="Fraction of a mutation to be considered as a precision for branch length estimation (default 0.001, which means branch lengths estimated up to a 1000th of a mutation precision).",  type=float, default=0.0001)
...
parser.add_argument("--thresholdLogLKoptimizationTopology",help="logLK difference threshold to consider a logLk close to optimal when deciding for which possible placements to perform branch length optimization.",  type=float, default=12.0)
parser.add_argument('--assignmentFile',default="", help='give path and name to a .csv file containing reference sample names and their lineage assignment. When using this option, option --inputTree should also be used. Then MAPLE will assign all samples in the tree to a lineage following the reference lineages file. Each sample is assigned a lineage same as its closest reference parent.')
...
if assignmentFile!="":
	if not os.path.isfile(inputTree):
		...
		exit()
	if not os.path.isfile(assignmentFile):
...
def estimateBranchLengthWithDerivative(probVectP,probVectC,mutMatrix,useRateVariation=False,mutMatrices=None):
		...
		if entry2[0]==5: # case entry1 is N
			...
			if entry1[0]==4: # case entry1 is R	
				if entry2[0]==4:
					...
					if coeff1<0.0:
						...
						# if coeff0<0.0:
						# 	print("Negative a coefficient "+str(coeff0)+" from R->O with entries" )
						# 	print(entry1)
						# 	print(entry2)
						ais.append(coeff0)
					pos+=1
					...
					if len(entry1)==4:
						...
						if contribLength:
							...
							coeff0=0.0
					# if coeff0<0.0:
					# 	print("Negative a coefficient "+str(coeff0)+" from R->A with entries" )
					# 	print(entry1)
					# 	print(entry2)
					ais.append(coeff0)
					pos+=1
			...
			# entry1 is of type "O"
				...
				if coeff1<0.0:
					...
					coeff0=coeff0/coeff1
					# if coeff0<0.0:
					# 	print("Negative a coefficient "+str(coeff0)+" from O->A with entries" )
					# 	print(entry1)
					# 	print(entry2)
					ais.append(coeff0)
				pos+=1
					...
					if entry2[0]<5: #entry2 is a nucleotide
						...
						if len(entry1)==4:
							...
							if contribLength:
								...
								coeff0=0.0
						# if coeff0<0.0:
						# 	print("Negative a coefficient "+str(coeff0)+" from A->C with entries" )
						# 	print(entry1)
						# 	print(entry2)
						# 	exit()
						ais.append(coeff0)
						...
						if coeff1<0.0:
							...
							coeff0=coeff0/coeff1
							# if coeff0<0.0:
							# 	print("Negative a coefficient "+str(coeff0)+" from A->O with entries" )
							# 	print(entry1)
							# 	print(entry2)
							# 	exit()
							ais.append(coeff0)
	...
	if n==0:
		...
		# remove
		# if vDown==float("inf"):
		# 	print("Found infinite derivative, tDown "+str(tDown)+" c1 "+str(c1)+" n/c1 "+str(n/c1)+" ais")
		# 	print(ais)
		# 	print(probVectP)
		# 	print(probVectC)
		# 	exit()
		...
		for ai in ais:
			vUp+=1.0/(ai+tUp)
		# if vUp==float("inf"):
		# 	print("Found infinite derivative, tDown "+str(tDown)+" vDown "+str(vDown)+" c1 "+str(c1)+" n/c1 "+str(n/c1)+" ais")
		# 	print(ais)
		# 	print(" tUp "+str(tUp))
		# 	print(probVectP)
		# 	print(probVectC)
	...
	while tDown-tUp>minBLenSensitivity:
		...
		# remove
		# if vMiddle==float("inf"):
		# 	print("Found infinite derivative, tDown "+str(tDown)+" tUp "+str(tUp)+" c1 "+str(c1)+" n/c1 "+str(n/c1)+" vDown "+str(vDown)+" vUp "+str(vUp)+" ais")
		# 	print(ais)
		# 	print(" tMiddle "+str(tMiddle)+"  ")
		# 	print(probVectP)
		# 	print(probVectC)
		# 	exit()
...
def updateBLenOld(nodeList,node,mutMatrix,useRateVariation=False,mutMatrices=None):
	cNode=node
	node=node.up
	if cNode==node.children[0]:
		vectUp=node.probVectUpRight
...
def findBestParentTopology(node,child,bestLKdiff,removedBLen,mutMatrix,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,useRateVariation=False,mutMatrices=None):
	...
	#bestBranchLengths=(node.dist,node.children[1-child].dist,removedBLen)
	bestNodes=[]
	nodesToVisit=[]
	...
	if not bestNodes:
		# if (not originalBLens[0]) and (not originalBLens[1]):
		# 	print("inside findBestParentTopology(), no bestNodes, found")
		# 	print(originalBLens)
		# 	print(originalLK)
				...
				if testNonsensicalBLens:
					...
					if (not bestBottomLength) and (not bestTopLength):
						...
						if newMidVector==None:
							...
							print("AppendingCost "+str(appendingCost)+" initialCost "+str(initialCost)+" newPartialCost "+str(newPartialCost)+" optimizedScore "+str(optimizedScore))
							exit()
						# print("Found 0 top and 0 bottom lenghts in findBestParentTopology():")
						# print(bestBranchLengths)
						# print(upVect)
						# print(downVect)
						# print(removedPartials)
						# print("Vector that might be None")
						# print(newMidVector)
	...
	if not BLenHaveBeenOptimized:
		...
		# if (not bestBranchLengths[0]) and (not bestBranchLengths[1]):
		# 	print("inside findBestParentTopology(), not BLenHaveBeenOptimized, found")
		# 	print(bestBranchLengths)
		# 	print(bestScore)
		# 	print(removedPartials)
	# elif (not bestBranchLengths[0]) and (not bestBranchLengths[1]):
	# 	print("inside findBestParentTopology(), yes BLenHaveBeenOptimized, found")
	# 	print(bestBranchLengths)
	# 	print(bestScore)
	# 	print(removedPartials)
...
def findBestParentForNewSample(root,diffs,sample,mutMatrix):
	...
	#if debugging:
	#	print("Appending cost at root "+str(bestLKdiff))
	nodesToVisit=[]
	...
	while nodesToVisit:
		...
		if t1.dist and t1.up!=None: # try first placing as a descendant of the mid-branch point of the branch above the current node.
			...
			if LKdiff>=bestLKdiff:
				...
				#if debugging:
				#	if bestLKdiff>0.5:
				#		print("Found positive appending cost at initial round "+str(bestLKdiff))
...
# 		# remove
# 		#if not (ai+t):
# 		#	print("found 0.0 denominator, ai "+str(ai)+" t "+str(t))
# 		#	return float("inf")
# 		derivative+=1.0/(ai+t)
# 	return derivative
...
def placeSubtreeOnTree(node,newPartials,appendedNode,newChildLK,bestBranchLengths,mutMatrix,useRateVariation=False,mutMatrices=None):
	...
	if newInternalNode.probVect==None:
		# print("Problem in placeSubtreeOnTree(), probVect is None")
		# print(node.probVect)
		# print(bestDownLength)
		# print(newPartials)
		# print(bestAppendingLength)
	...
	newInternalNode.probVectUpRight=mergeVectorsUpDown(vectUp,bestUpLength,newPartials,bestAppendingLength,mutMatrix,useRateVariation=useRateVariation,mutMatrices=mutMatrices)
	if newInternalNode.probVectUpRight==None:
		# print("Problem in placeSubtreeOnTree(), probVectUpRight is None")
		# print(vectUp)
		# print(bestUpLength)
		# print(newPartials)
		# print(bestAppendingLength)
	...
	newInternalNode.probVectUpLeft=mergeVectorsUpDown(vectUp,bestUpLength,node.probVect,bestDownLength,mutMatrix,useRateVariation=useRateVariation,mutMatrices=mutMatrices)
	if newInternalNode.probVectUpLeft==None:
		# print("Problem in placeSubtreeOnTree(), probVectUpLeft is None")
		# print(vectUp)
		# print(bestUpLength)
		# print(node.probVect)
		# print(bestDownLength)
...
def traverseTreeForTopologyUpdate(node,mutMatrix,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement,useRateVariation=False,mutMatrices=None):
	...
	if node.up!=None:
		...
		originalLK=appendProbNode(vectUp,node.probVect,bestCurrenBLen,mutMatrix,useRateVariation=useRateVariation,mutMatrices=mutMatrices)
			...
			#print("Found an infinite cost of originalLK "+str(originalLK)+" using appendProbNode()")
			#print(vectUp)
			#print(node.probVect)
		...
		if bestCurrentLK<thresholdTopologyPlacement:
			...
			if bestCurrenBLen or node.dist:
				...
				if debugging:
					...
					#while root.up!=None:
					#	root=root.up
					#reCalculateAllGenomeLists(root,mutMatrix, checkExistingAreCorrect=False)
					#newTreeLK=calculateTreeLikelihood(root,mutMatrix)
					#print("Total tree likelihood with new branch length: "+str(newTreeLK))
					#node.dist=originalBLen
					#reCalculateAllGenomeLists(root,mutMatrix, checkExistingAreCorrect=False)
...
### testing
#print(str(countLeavesInTree(t1))+" leaves in tree after initial tree creation.")
...
### testing
#print(str(countLeavesInTree(t1))+" leaves in tree after full SPR.")
