...
#TODO @Andrea this is the option to trigger the parallelizable SPR search - it turns off the proper implementation of the SPR moves
parser.add_argument("--onlySearchSPRmoves", help="This option turns off proper SPR improvements; instead MAPLE will just calculate a list of feasible SPR moves.", action="store_true")
...
onlySearchSPRmoves=args.onlySearchSPRmoves
...
# try to find a re-placement of a dirty node of the tree to improve the topology.
# This function does not modify the tree, it just looks for improvements and returns the best suggested improvement found.
# To find the best location of the new re-attachment, we traverse the tree starting at the current attachment node, and for each node visited we evaluate the reattachment.
# To avoid traversing the whole tree for each SPR move, we use stopping conditions similar to those used in the initial sample placement process.
def traverseTreeForTopologySearch(node,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement):
	# has the branch length been updated so far? If we change the branch length above the current node, even if we don't perform any SPR, we still need to update the genome lists in the tree.
	bLenChanged=False
	totalImprovement=0.0
	# we avoid the root node since it cannot be re-placed with SPR moves
			...
			if bestLKdiff+thresholdTopologyPlacement>bestCurrentLK:
				...
				if topologyUpdated:
					...
					if debugging:
						...
						print("for supposed improvement "+str(totalImprovement))
					#newRoot = cutAndPasteNode(node,bestNodeSoFar,bestBranchLengths,bestLKdiff)
					return True, (bestNodeSoFar, bestBranchLengths, bestLKdiff), totalImprovement
					#bLenChanged=False
		if (not topologyUpdated) and bLenChanged:
			return False, bestCurrenBLen, (bestCurrentLK-originalLK)
			# if debugging:
			# 	print("Changing branch length (pos 3) from "+str(node.dist)+" to "+str(bestCurrenBLen)+" at node "+str(node)+" with children ")
			# 	print(node.children)
			# node.dist=bestCurrenBLen
...
#TODO @Andrea, this below is the function that needs to be parallelized, that is, we need to call traverseTreeForTopologySearch() on separate processors.

#traverse the tree (here the input "node" will usually be the root), and for each dirty node ancountered, call traverseTreeForTopologySearch() 
# to loof for an SPR move that would cut the subtree rooted at this dirty node and re-append it elsewhere - in this function the SPR move is not executed, just searched!
def startTopologySearch(node,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement):
	nodesToVisit=[node]
	numNodes=0
	listOfProposedSPRmoves=[]
	while nodesToVisit:
		newNode=nodesToVisit.pop()
		for c in newNode.children:
			nodesToVisit.append(c)
		if newNode.dirty and newNode.replacements<=maxReplacements:
			newNode.dirty=False
			results=traverseTreeForTopologySearch(newNode,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,thresholdTopologyPlacement=thresholdTopologyPlacement)
			if results!=None:
				listOfProposedSPRmoves.append((newNode,results[0],results[1],results[2]))
			numNodes+=1
			if (numNodes%1000)==0:
				print("Searched SPR moves for "+str(numNodes)+" nodes.")
	return listOfProposedSPRmoves
...
#TODO @Andrea this is where the parallelizable SPR search is called

#just search for topology improvements without actually implementing them
if onlySearchSPRmoves:
	print("Starting topological impromevement search traversal (the topology won't actually be changed) ")
	start=time()
	if inputTree=="" or largeUpdate:
		setAllDirty(t1)

	#TODO @Andrea it might make sense to write a parallelized version of the function below, and compare its performance with this sequential version.
	proposedSPRmoves=startTopologySearch(t1)

	from operator import itemgetter
	proposedSPRmoves=sorted(proposedSPRmoves,key=itemgetter(3),reverse=True)
	timeForSearchingTopology=(time()-start)
	print("Time for this traversal of the tree to search SPR moves: "+str(timeForSearchingTopology))
	print("Found "+str(len(proposedSPRmoves))+" proposed SPR moves, the best 10 are ")
	print(proposedSPRmoves[:10])
	print("And the last are ")
	print(proposedSPRmoves[-10:])
