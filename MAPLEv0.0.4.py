...
#TODO instead of attempting 2 different branch lengths for attempting an initial placement, try only one branch, and then if the final branch is much smaller/bigger than the initial one, 
# try placement again with a different branch length starting from the placement point (basically an SPR search for the just placed tip).
...
#TODO try to improve parameter values so that it would run smoothly also with bacteria.
...
parser.add_argument("--thresholdLogLK",help="logLK difference threshold to consider a logLk close to optimal.",  type=float, default=30.0)
parser.add_argument("--thresholdLogLKtopology",help="logLK difference threshold to consider a logLk close to optimal when looking for topology improvements.",  type=float, default=10.0)
...
parser.add_argument("--numTopologyImprovements",help="Number of times we traverse the tree looking for topological improvements.",  type=int, default=2)
